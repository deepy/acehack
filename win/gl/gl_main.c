...
struct window_procs sdlgl_hardw_procs = 
{
    ...
    Sdlgl_player_selection,
    0, //Sdlgl_game_mode_selection,
    Sdlgl_askname,
    Sdlgl_get_nh_event,
    Sdlgl_exit_nhwindows,
    Sdlgl_suspend_nhwindows,
    Sdlgl_resume_nhwindows,
    Sdlgl_create_nhwindow,
    Sdlgl_clear_nhwindow,
    Sdlgl_display_nhwindow,
    Sdlgl_destroy_nhwindow,
    Sdlgl_curs,
    0, //Sdlgl_suppress_more,
    Sdlgl_putstr,
    0, //Sdlgl_putstr_colored,
    Sdlgl_display_file,
    Sdlgl_start_menu,
    Sdlgl_add_menu,
    0, //Sdlgl_add_menu_colored,
    Sdlgl_end_menu,
    Sdlgl_select_menu,
    Sdlgl_message_menu,
...
static int choose_best_video_mode(SDL_Rect **modes)
{
  ...
  for (i=0; modes[i]; i++)
  {
    ...
    {
      if (w < 800) dist = dist * 3 / 2;
      if (h < 600) dist = dist * 3 / 2;
    }
...
static int convert_keysym(int sym, int unicode, int mod)
{
  ...
  switch (sym)
  {
    ...
    /* cursor keys & keypad */
    case SDLK_UP:
      return shift ? *key_for_cmd("#north") : *key_for_cmd("#northfar");
     
    case SDLK_DOWN:
      return shift ? *key_for_cmd("#south") : *key_for_cmd("#southfar");
     
    case SDLK_LEFT:
      return shift ? *key_for_cmd("#west") : *key_for_cmd("#westfar");

    case SDLK_RIGHT:
      return shift ? *key_for_cmd("east") : *key_for_cmd("#eastfar");
