...
static unsigned int good_random(void);
...
/* AES/SHA256 code here. Copy+pasted from LibTomCrypt, which is in public 
   domain. */
...
/***
  ...
  Make a call to the entropy collector when added.
***/
...
static unsigned int good_random(void)
{
	...
	/* The initialization characters don't really matter. (I think). Have 
	   32 of them though. */
	...
	{
		output_number = 0;
		AES_encrypt((unsigned char*) counter, 
	...
	/* Increment counter. It's has many bits so it needs some fiddling. */
	++counter[0];
	if (counter[0] == 0)
	{
		++counter[1];
